...
for i, val in os.environ.items():
    ...
    # but prefixed value has the higher priority
    for s in ENV_VARS:
...
def set_prod_settings(g, env):
    ...
    g.setdefault('DEFAULT_FROM_EMAIL', '{env}-{{cookiecutter.name}}@{{cookiecutter.tld_domain}}')
    g['CORS_ORIGIN_WHITELIST '] = (
        f'{env}-terralego-{{cookiecutter.name}}.{{cookiecutter.tld_domain}}',  #noqa
    )
    g['ALLOWED_HOSTS'] = [
        f'{env}-terralego-{{cookiecutter.name}}.{{cookiecutter.tld_domain}}',
